...
const experimentSeed: TablesInsert<"experiments"> = {
  experiment_id: "seed_experiment_1",
...
const insertIfMissing = async <T extends SeedTables>(
  ...
  const uniqueValue = payload[uniqueKey as keyof TablesInsert<T>];
  ...
  const { data: existing, error: selectError } = await supabase
    .from(table)
    .select(uniqueKey)
    .eq(uniqueKey, String(uniqueValue))
    .maybeSingle();
